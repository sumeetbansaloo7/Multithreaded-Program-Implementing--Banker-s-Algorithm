...
sem_t semaphore;                   // for threads syncronization
...
int main()
{
    ...
    sem_init(&semaphore, 0, no_of_process);
    ...
    for (int i = 0; i < no_of_process; i++)
    {
        pthread_create(&processes[i], NULL, thread_function, &safe_sequence[i]);
    }
